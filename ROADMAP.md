...
| 32a | AniList scores for thin-evidence anime | Good to have | Considered | — | The only second rating source that passes all three tests: fills a gap IMDb is weak on, needs no key or server, and is reachable. Measured: "Hajime no Ippo" is 8.5 from **91 IMDb votes** but 87/100 from **143,693 AniList users**; 7 of 7 homepage anime matched. Held back by two things — AniList is 0–100 against IMDb's 0–10, so a shared badge would conflate two different voting populations without visible provenance; and the limit is 30 requests/minute, so it only works scoped to titles `basics` marks as Japanese animation *with* a thin IMDb vote count, cached permanently |
