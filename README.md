...
- **`content.js`** runs inside the Netflix page. Netflix builds title cards
  ...
  screen — scrolling past three rows shouldn't spend quota on thirty.
...
- **Netflix's HTML is not a contract.** The extension reads titles from class
  names like `.fallback-text` that Netflix changes every few months. When it
  silently stops badging, that's almost always why — the fix is updating
  `CARD_SELECTORS` and `titleFromCard()` in `content.js`.
